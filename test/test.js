...
tape( 'the function returns an accumulator function', function test( t ) {
	t.strictEqual( typeof incrcovariance(), 'function', 'returns expected value' );
	t.end();
});

tape( 'the function returns an accumulator function (known means)', function test( t ) {
	t.strictEqual( typeof incrcovariance( 3.0, -5.0 ), 'function', 'returns expected value' );
	t.end();
});
...
tape( 'if not provided an input value, the accumulator function returns the current unbiased sample covariance', function test( t ) {
	...
	t.strictEqual( acc(), -1.7, 'returns the current accumulated unbiased sample covariance' );
	t.end();
});
...
tape( 'if not provided an input value, the accumulator function returns the current unbiased sample covariance (known means)', function test( t ) {
	...
	t.strictEqual( acc(), -1.1333333333333335, 'returns the current accumulated unbiased sample covariance' );
	t.end();
});
